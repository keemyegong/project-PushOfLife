...
import com.example.pushoflife.SensorForegroundService
...
class AedLocationActivity : AppCompatActivity(), TMapGpsManager.OnLocationChangedListener, TMapView.OnClickListenerCallback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // FusedLocationProviderClient 초기화 (위치 정보를 제공)
        fusedLocationClient = LocationServices.getFusedLocationProviderClient(this)
        // 화면이 절대 꺼지지 않도록 설정
        window.addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON)

        // tMapViewContainer와 currentLocationButton UI 요소 참조 가져오기
        ...
        // TMapGpsManager 초기화 (위치 갱신을 위한 설정)
//        tMapGpsManager = TMapGpsManager(this).apply {
//            minTime = 500 // 위치 갱신 간격 (밀리초 단위)
//            minDistance = 10f // 위치 갱신 최소 거리 (미터 단위)
//        }

        // TMapView 초기화 및 API 키 설정
        tMapView = TMapView(this).apply {
            setSKTMapApiKey(BuildConfig.T_MAP_KEY)
        }
        tmapViewContainer.addView(tMapView)

        // TMapView가 초기화된 후에 위치를 업데이트할 수 있도록 OnMapReadyListener 설정
        tMapView.setOnMapReadyListener {
            // 위치 권한을 확인하고 초기 위치 이동
        ...
        // 응급상황 버튼
        val emergencyButton = findViewById<ImageButton>(R.id.emergencyButton).apply {
            visibility = View.GONE // 초기에는 버튼을 숨김
        }
        val emergencyText = findViewById<TextView>(R.id.emergencyText).apply {
            visibility = View.GONE // 초기에는 텍스트 숨김
        }
